...
<div class="card p-2" style="border-left: 8px solid var(--primary-blue);">
    ...
    <p class="text-sm text-muted">Terakhir login: <span class="font-bold color-primary"><?php echo htmlspecialchars($admin['last_login'] ?? 'Baru saja'); ?></span></p>
</div>

<!-- Stats Overview -->
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
    <div class="card text-center p-2">
        <div style="font-size: 3rem; font-weight: 800; line-height: 1;" class="color-primary"><?php echo $stats['total_waifus']; ?></div>
        <p class="text-xs font-black opacity-06 text-uppercase">Total Waifu</p>
    </div>
    <div class="card text-center p-2">
        <div style="font-size: 3rem; font-weight: 800; line-height: 1;" class="color-danger"><?php echo $stats['total_players']; ?></div>
...
<div class="card p-2 mb-2">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 1rem;">
        ...
        foreach ($allTiers as $tier): 
            $count = $stats['tiers'][$tier] ?? 0;
        ?>
            <div class="text-center p-1" style="background: var(--bg-light); border-radius: 12px; border: 1px solid var(--text-dark);">
                <div class="tier-badge tier-<?php echo strtolower($tier); ?>" style="position: static; display: inline-block; margin-bottom: 5px;"><?php echo $tier; ?></div>
